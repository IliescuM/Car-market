import { Alert, Button, Center, Grid, Group, Loader, Modal, Text } from "@mantine/core"
import { IconAlertCircle, IconCheck } from "@tabler/icons"
import { useEffect, useState } from "react"
import { useLocation } from "react-router-dom"
import { useCarsQuery } from "../../hooks/queries/useCarsQuery"
import { CarCard } from "./components/CarCard"
import { CreateCar } from "./components/CreateCar/CreateCar"

export const Cars = () => {
    //const { isLoading, isError, cars } = useCars()
    const [opened, setOpened] = useState(false);
    const { isLoading, isError, data: cars } = useCarsQuery()
    const location = useLocation();

    if (isLoading) {
        return <Center>
            <Loader size="xl" />
        </Center>
    }
    if (isError) {
        return <div>
            <Alert icon={<IconAlertCircle size={16} />} title="Error!" color="red">
                Something terrible happened! You made a mistake and there is no going back, your data was lost forever!
            </Alert>
        </div>
    }
    return <div>

        {/* {location.state.alertDelete && <Alert icon={<IconCheck size={16} />} title="Succces!" color="teal">
            You deleted the car
        </Alert>} */}
        <div>
            <Modal
                opened={opened}
                onClose={() => setOpened(false)}
                title="Create new Car"
            >
                {/* Modal content */}
                <CreateCar></CreateCar>
            </Modal>

            <Group position="center">
                <Button mb={20} onClick={() => setOpened(true)}>Add Car</Button>
            </Group>
        </div>
        <Grid sx={{ gridAutoRows: "1fr" }} gutter="xl">
            {cars.map(car => (
                <Grid.Col sm={4} md={4} key={car._uuid}>
                    <CarCard car={car} />
                </Grid.Col>
            ))}
        </Grid>
    </div>
}